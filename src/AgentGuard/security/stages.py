...
DEFAULT_TOOL_ALLOWLIST: frozenset[str] = frozenset({"Read", "Write", "Edit", "Bash", "Grep", "Glob"})
...
def stage2_allowed_tools(frontmatter: dict[str, Any], override: Sequence[str] | None) -> list[Finding]:
    ...
    allowlist: frozenset[str] = frozenset(override) if override is not None else DEFAULT_TOOL_ALLOWLIST
    findings: list[Finding] = []
        ...
        if not isinstance(tool, str):
            findings.append(
                _f(
                    ...
                    f"frontmatter:allowed-tools[{idx}]",
                    "AGRD-S-201",
                )
            )
            continue
...
def stage4_aidefence(body: str, skill_dir: Path, threshold: float) -> list[Finding]:
    ...
    if references.exists() and references.is_dir():
        for ref in sorted(references.rglob("*.md")):
            try:
                _scan_block(findings, ref.read_text(encoding="utf-8", errors="replace"), str(ref), threshold)
            except OSError as exc:
                findings.append(
                    _f(
                        ScannerStage.AIDEFENCE_INJECTION,
                        Severity.MEDIUM,
                        f"Could not read reference {ref.name}: {exc}",
                        "Ensure the reference is readable.",
                        str(ref),
                        "AGRD-S-401",
                    )
                )
...
def _scan_block(sink: list[Finding], text: str, location: str, threshold: float) -> None:
    ...
    sink.append(
        _f(
            ...
            "Remove or rephrase the flagged passage; if legitimate, review with `--allow-suspicious-skill`.",
            location,
            "AGRD-S-400",
        )
    )
...
def stage5_signature(skill_dir: Path, frontmatter: dict[str, Any]) -> tuple[str, list[Finding]]:
...
def stage6_marketplace(skill_dir: Path, skill_name: str, override: Sequence[str] | None) -> list[Finding]:
