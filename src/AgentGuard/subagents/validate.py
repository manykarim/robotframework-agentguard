    ...
    if isinstance(card, Path):
        try:
            ...
            raise AgentCardInvalid(
                f"could not read agent card file {card}: {exc}"
            ) from exc
        except json.JSONDecodeError as exc:
            raise AgentCardInvalid(
                f"agent card file {card} is not valid JSON: {exc}"
            ) from exc
        ...
        if stripped.startswith("{"):
            try:
                ...
                raise AgentCardInvalid(
                    f"agent card string is not valid JSON: {exc}"
                ) from exc
        else:
            path = Path(stripped)
            if not path.exists():
                raise AgentCardInvalid(
                    f"agent card input is neither JSON nor an existing file: {card!r}"
                )
            try:
                raw = json.loads(path.read_text())
            except json.JSONDecodeError as exc:
                raise AgentCardInvalid(
                    f"agent card file {path} is not valid JSON: {exc}"
                ) from exc
    elif isinstance(card, dict):
        raw = card
    else:
        raise AgentCardInvalid(
            f"unsupported agent card input type: {type(card).__name__}"
        )

    if not isinstance(raw, dict):
        raise AgentCardInvalid(
            f"agent card root must be an object, got {type(raw).__name__}"
        )
...
def assert_card_required_fields(card: AgentCard) -> None:
    ...
    if missing:
        raise AgentCardInvalid(
            f"AgentCard missing required field(s): {', '.join(missing)}"
        )
    for i, skill in enumerate(card.skills):
        if not skill.id or not skill.name:
            raise AgentCardInvalid(
                f"AgentCard skill[{i}] is missing id or name "
                f"(got id={skill.id!r}, name={skill.name!r})"
            )
